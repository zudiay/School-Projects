...
int main(int argc, char* argv[]){
    ...
    int input_size = N*(A+1);   // number of data to be read
    float input[input_size];    // the N lines of input

    int slave_input_size= input_size / (P-1);   // number of data per slave
    float slave_input[slave_input_size];        // the input partition for each slave

    float scatter[slave_input_size*P];  // data to be partitioned

    float weights[A];   // weights for each feature per slave
    for(int i=0; i<A; i++)
        weights[i]=0;

    int arr[P*T];   // total chosen features
    for(int i = 0; i<P*T; i++)
        arr[i]= -1;
    ...
    // If it's master processor, reads further lines from input file
    if(rank==0){
        float num;
    ...
    masterSignal = 1;
    while(masterSignal){
        // If this is a slave, performs the relief algorithm
        if(rank!= 0){
            for(int i=0; i<M; i++){ // does M iterations

                // each iteration has a different target (sequentially)
                int target = i*(A+1);   // start index of target instance
                ...
                float max[A];   // max array holds the maximum value of each feature
                float min[A];   // min array holds the minimum value of each feature
                for(int j=0; j<A; j++){ // initially make them equal to the first instance's features
                    max[j]=slave_input[j];
                    min[j]=slave_input[j];
                }
                ...
                // Traverses all instances and calculates manhattan distance between each instance and the target
                for(int j=0; j<N/(P-1); j++){
                    float distance = 0;
                    // Compares each feature, checks min and max situations
                    // and adds absolute difference to manhattan distance
                    for(int k=0; k<A; k++){
                    ...
                    if(j!=i){
                        if(target_flag==flag && distance<nearest_hit_distance){ // hit
                            nearest_hit_distance = distance;
                            nearest_hit = j;
                        }
                        else if(target_flag!=flag && distance<nearest_miss_distance) { // miss
                ...
                // Calculations for this iteration are completed. Weight can be updated according to Relief algorithm.
                for(int a=0; a<A; a++){
                    weights[a] = weights[a]
            ...
            // Iterations are completed, top T features can be selected.
