...
;;;;; FUNCTIONS ABOUT ADJACENT & NEIGHBOR SITUATIONS;;;;;
...
;;;;; FUNCTIONS ABOUT LIST OPERATIONS ;;;;;
...
;;;;; FUNCTIONS ABOUT SOLUTION CONDITIONS ;;;;;
...
;; NOT USED BUT NOT REMOVED IN CASE THERE IS AN ERROR AND MINOR CHANCES ARE REQUIRED BY THE ASSISTANTS
; update possible empty places when new tent is placed
(define (UPDATE-POSSIBLE row col  tents possible)
  (cond [(null? possible) '() ]
        [(IS-SAFE row col tents (car possible)) (cons (car possible) (UPDATE-POSSIBLE row col  tents (cdr possible)))]
        [else (UPDATE-POSSIBLE row col  tents (cdr possible))] ))
...
;; NOT USED BUT NOT REMOVED IN CASE THERE IS AN ERROR AND MINOR CHANCES ARE REQUIRED BY THE ASSISTANTS
; return if it is safe to place a tent at coordinate pair
(define (IS-SAFE  row col tents pair)
  (cond [(<= (GET-NTH (car pair) row)  0) #f]
        [(<= (GET-NTH (cadr pair) col)  0) #f]
        [(ADJACENT-WITH-LIST pair tents) #f]
        [else #t]))
...
;; NOT USED BUT NOT REMOVED IN CASE THERE IS AN ERROR AND MINOR CHANCES ARE REQUIRED BY THE ASSISTANTS
; removes the cells whose row or col constraints force 0 tents 
(define (REMOVE-EMPTY row col listpos)
  (cond ((null? listpos) '())
        ((not(IS-PLACE row col (car listpos))) (REMOVE-EMPTY row col (cdr listpos)))
        (else (cons (car listpos) (REMOVE-EMPTY row col (cdr listpos))))))

;; NOT USED BUT NOT REMOVED IN CASE THERE IS AN ERROR AND MINOR CHANCES ARE REQUIRED BY THE ASSISTANTS
; takes the first element in list that does not return false when the function is applied, returns result
(define (RETURN-FIRST-NOT-FALSE f list)
  (cond [(null? list) #f]
        [(f (car list)) (f (car list))]
        [else (RETURN-FIRST-NOT-FALSE f (cdr list))]))
...
;; NOT USED BUT NOT REMOVED IN CASE THERE IS AN ERROR AND MINOR CHANCES ARE REQUIRED BY THE ASSISTANTS
;backtracking, removing empty rows and columns at each insertion
(define (HELPER2 row col trees tents possible)
  (cond [(SOLUTION trees tents row col) tents]
        [(null? possible) #f]
        [(not (SOLVABLE row (GET-ALL-NTH possible 1) 1) ) #f]
        [(not (SOLVABLE col (GET-ALL-NTH possible 2) 1) ) #f]
        [(if (IS-PLACE row col (car possible))
         (or (HELPER2 (DECREASE-NTH-ONE row (caar possible))
                      (DECREASE-NTH-ONE col (cadar possible))
                      trees (append tents (list (car possible)))
                     (REMOVE-EMPTY (DECREASE-NTH-ONE row (caar possible)) (DECREASE-NTH-ONE col (cadar possible)) (DELETE-ADJ (cdr possible) (car possible))))
              (HELPER2 row col trees tents (cdr possible)))
          (HELPER2 row col trees tents (cdr possible)))]
        [else #f]))

;; NOT USED BUT NOT REMOVED IN CASE THERE IS AN ERROR AND MINOR CHANCES ARE REQUIRED BY THE ASSISTANTS
;backtracking, removing empty rows and columns, and also deleting adjacents at each insertion
(define (HELPER3 row col trees tents possible)
  (cond [(SOLUTION trees tents row col) tents]
        [(null? possible) #f]
        [(not (SOLVABLE row (GET-ALL-NTH possible 1) 1) ) #f]
        [(not (SOLVABLE col (GET-ALL-NTH possible 2) 1) ) #f]
        [(if (IS-PLACE row col (car possible))
         (or (HELPER3 (DECREASE-NTH-ONE row (caar possible))
                      (DECREASE-NTH-ONE col (cadar possible))
                      trees (append tents (list (car possible)))
                      (UPDATE-POSSIBLE (DECREASE-NTH-ONE row (caar possible)) (DECREASE-NTH-ONE col (cadar possible))   (append tents (list (car possible)))(cdr possible)) )
              (HELPER3 row col trees tents (cdr possible)))
          (HELPER3 row col trees tents (cdr possible)))]
        [else #f]))
